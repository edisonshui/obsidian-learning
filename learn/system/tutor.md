...
- **Decide where the correct answer goes before writing the options, and log it.** Every logged multiple-choice check carries a `key: <slot>/<count> — options: <opt1> / <opt2> / <opt3>` field (`records.md`): the slot the correct option landed in, out of how many, then every option verbatim in slot order. `learn-status.py`'s G4 gate reads the slot sequence across a subject's session notes and reports a repeated or over-used slot after the fact, so the constraint lives in the field and the gate, not in memory. The option text is what lets `--semantic` ask the other half of this rule — whether the options could be picked without reading the question — which no count can answer. A learner who has noticed a positional pattern could pass every check without knowing the material, and the evidence line written afterward would say they knew it — that is the failure the gate exists to catch.
