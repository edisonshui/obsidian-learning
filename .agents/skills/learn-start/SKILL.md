...
Start a learning project for the subject slug in the user's request, for example `$learn-start pointers` or "start learning pointers." Treat any words after the slug as a goal hint. If the user gave a topic but no slug, make a short lowercase hyphenated slug from it and tell them what you chose.
...
Once the slug is known, run `python3 .codex/hooks/learning.py select --subject <slug>` to route this conversation to the subject's Codex log. If hooks are unavailable, continue with the learning records.

## 1. Set up the subject folder

- Slug: lowercase, hyphens, taken from the user request. If `learn/subjects/<slug>/record.md` already exists, stop and say so; the right command is `$learn-resume <slug>`.
...
- Probe each strand by binary search (see *Checks and quizzes* in `tutor.md`). Budget 10–15 minutes, about 12–20 questions total. Use a question picker for multiple choice when available; free-response in chat where a choice would give the answer away.
...
- Run the `fact-checker` custom subagent with the list of factual claims the plan rests on (definitions, rules, API facts, numbers). Fix what it marks wrong; list what it marks uncertain under *Review notes*.
- Run the `plan-reviewer` custom subagent with the paths of `plan.md`, `record.md`, and `learn/me/preferences.md`. Apply its fixes or say in *Review notes* why not.

If subagents are unavailable, perform both reviews yourself and say so.
...
Run `python3 .claude/hooks/learn-status.py` so `learn/Dashboard.md` and this subject's `progress.md` pick up the new subject. Never hand-write a row into either; they are generated files and the next run overwrites anything added by hand.
